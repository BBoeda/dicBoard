...
const BoardList = () => {
    ...
    const addRow = () => {
        const newRow = {code: 'CODE1', confirm: 'Y', id: tableData.length + 1};
    ...
    return (<div>
        ...
        <div className={"div-table-board-list"}>
            <table className={"table-board-list"}>
                ...
                {tableData.map((row, index) => (<tr key={row.id}>
                    ...
                    <td>
                        <select name="code" onChange={(event) => handleChange(row.id, event)}>
                            <option value={"CODE1"}>생활</option>
                            <option value={"CODE2"}>교육</option>
                            <option value={"CODE3"}>군사</option>
                            <option value={"CODE4"}>금융</option>
                        </select>
                    </td>
                    <td>
                        <select name="confirm" value={row.confirm}
                                onChange={(event) => handleChange(row.id, event)}>
                ...
                {boardList.map((board, index) => (<tr key={index}>
        ...
        <div className="page_wrap">
            ...
            {pageList.map((page, index) => (
                <button style={{backgroundColor: selPage === page ? '#F38A23FF' : 'initial'}} key={index} onClick={onClick} value={page}>
                {page}
                </button>))}
